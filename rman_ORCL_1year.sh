...
# to execute an archival full backup with additional archivelog
#
...
START_SCN=$( sqlplus -S -L /nolog <<eof
set heading off
set pagesize 0
set feedback off
connect / as sysdba
select trim(to_char(min( FIRST_CHANGE# - 1 ))) from v\$log where status in ('CURRENT', 'ACTIVE');
eof
)
echo "START_SCN=${START_SCN} - whis will be used to make an additional archivelog backup"


echo rman target / catalog ${RMANUSER}/XXXXXXXX@${RMAN_INSTANCE} cmdfile=\"`dirname $0`/rman_${ORACLE_UNQNAME}_ctrl.rcv\" `date +%Y%m%d%H%M` ${START_SCN}
rman target / catalog ${RMANUSER}/${RMANUSERPW}@${RMAN_INSTANCE} cmdfile=\"`dirname $0`/rman_${ORACLE_UNQNAME}_ctrl.rcv\" `date +%Y%m%d%H%M` ${START_SCN}
RETCODE=$?
