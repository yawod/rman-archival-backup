...
# to execute a controlfile backup
#
...
echo rman target / catalog ${RMANUSER}/XXXXXXXX@${RMAN_INSTANCE} cmdfile=\"`dirname $0`/rman_${ORACLE_UNQNAME}_ctrl.rcv\"
rman target / catalog ${RMANUSER}/${RMANUSERPW}@${RMAN_INSTANCE} cmdfile=\"`dirname $0`/rman_${ORACLE_UNQNAME}_ctrl.rcv\"
RETCODE=$?
