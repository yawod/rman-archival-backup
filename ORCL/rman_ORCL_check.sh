...
# to execute basic validation on rman backups (restore validate, crosscheck...)
#
...
echo rman target / catalog ${RMANUSER}/XXXXXXXX@${RMAN_INSTANCE} cmdfile=\"`dirname $0`/rman_${ORACLE_UNQNAME}_check.rcv\"
rman target / catalog ${RMANUSER}/${RMANUSERPW}@${RMAN_INSTANCE} cmdfile=\"`dirname $0`/rman_${ORACLE_UNQNAME}_check.rcv\"
RETCODE=$?
