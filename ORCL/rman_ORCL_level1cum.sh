...
# to execute an incremental, cumulative differential backup
#
...
echo rman target / catalog ${RMANUSER}/XXXXXXXX@${RMAN_INSTANCE} cmdfile=\"`dirname $0`/rman_${ORACLE_UNQNAME}_level1cum.rcv\"
rman target / catalog ${RMANUSER}/${RMANUSERPW}@${RMAN_INSTANCE} cmdfile=\"`dirname $0`/rman_${ORACLE_UNQNAME}_level1cum.rcv\"
RETCODE=$?
